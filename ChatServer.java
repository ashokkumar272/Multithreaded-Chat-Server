 ...
 * - Command processing (/list, /whisper, /exit)
...
public class ChatServer {
    ...
    public void broadcastMessage(Message message) {
        ...
        if (message.getType() == Message.MessageType.BROADCAST) {
            System.out.println("📢 [BROADCAST] " + message.getSender() + ": " + message.getContent());
        }
    }
    
    /**
     * Send a private message to a specific user (thread-safe)
     * 
    ...
    public boolean sendPrivateMessage(Message message) {
        synchronized (lock) {
            Map<String, PrintWriter> activeUsers = userManager.getActiveUsers();
            PrintWriter targetWriter = activeUsers.get(message.getRecipient());
            
            if (targetWriter != null) {
                String formattedMessage = "[PRIVATE from " + message.getSender() + "]: " + message.getContent();
                targetWriter.println(formattedMessage);
                
                // Log private message
                System.out.println("💬 [PRIVATE] " + message.getSender() + 
                                 " -> " + message.getRecipient() + ": " + message.getContent());
                
                return !targetWriter.checkError();
            }
